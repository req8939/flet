...
def main(page: ft.Page):
    ...
    def abrir_editor(u):
        ...
        lbl_edit_edad_valor.value = str(u['edad'])
    ...
    def filtrar_usuarios(e):
        ...
        resultado = list(
            filter(lambda u: u['nombre'].lower().startswith(busqueda) or u.get('apellido', '').lower().startswith(busqueda) or u.get('fecha_nacimiento', '').lower().startswith(busqueda), usuarios)
        )
    ...
    page.add(
        ft.Column(
            controls=[
                ...
                ft.Container(content=txt_search, padding=ft.padding.only(bottom=20)),
                txt_mensaje,
                contenedor_tabla,
                ft.Text("Version 1.0", size=10, weight="bold", color=ft.Colors.GREY_800, text_align="center")
            ],
